...
- When Authorization policy are created of type CUSTOM, ALLOW and Deny at the same time then CUSTOM is evaluated first then Deny then Allow.
- istio authorization policy also supports the AUDIT action to decide whether to log requests. In order for the audit to actually work a separate plugin must be installed. currently istio supports `Stackdriver` plugin.
- **gRPC**: istiod offers a gRPC service to take certificate signning requests (CSRs). gRPC is a robust open source RPC (Remote Procedure call) framework used to build scallable and fast APIs. gRPC is a communication protocol built on HTTP/2, unlike rest over HTTP/1, which is based on resources, gRPC is based on service definitions. you specify service definition in a format called protocol buffers.
